...
@section('content')
    ...
    <div class="row page-titles">
        <div class="col-md-5 col-8 align-self-center">
            <h3 class="text-themecolor">Colors Management</h3>
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <a href="javascript:void(0)">Dashboard</a>
                </li>
                <li class="breadcrumb-item active">Colors Management</li>
            </ol>
        </div>
    </div>
    ...
    <div class="modal fade" id="addEditModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('color.store') }}" method="post" id="addEditForm" class="floating-labels">
    ...
    <script>
        ...
        $(document).ready(function(e) {
            ...
            var table = $('#dataTable').DataTable( {
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('datatables.colors.list') }}",
                    "type": "GET"
                },
                "columns": [
                    {data: "title", name: "title" },
                    {data: "color", name: "color", render: function (data) {
                            return `<span style="background-color: ${data}; width: 50px; height: 50px;">${data}</span>`
                        } },
            ...
            $("body").on("click",".editRecord",function(e) {
                ...
                idHdnField.val(id);
                $("input[name=title]").val(data.title).parent().addClass('focused');
                $("input[name=color]").val(data.color);
                $('.asColorPicker-trigger').find('span').css('background', data.color);
                addEditModal.modal('show');
            });
            ...
            $("body").on("click",".deleteRecord",function(e) {
                e.preventDefault();
                var id = $(this).attr('id');
                var url = '{{ route('color.destroy', 'xxxx') }}';
